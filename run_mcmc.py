...
def main():
	...
	input_data_file=''
	output_chain_file=''
	start_parameters=np.array([1.258,13.339,0.345,11.962,11.918,0.01,-0.01])
	
	parser=OptionParser()

	parser.add_option('--ifile',type='string',help='input data file containing wp(rp)')
	parser.add_option('--covarfile',type='string',help='covariance matrix file for wp(rp)')
	parser.add_option('--ofile',type='string',help='output file to write chain samples to')

	parser.add_option('--start',nargs=7,type='float',help='alpha log(M1) sigma_log(M) log(M0) log(Mmin)')

	parser.add_option('--walkers',default=10,type='int',help='integer number of mcmc walkers')

	parser.add_option('--samples',default=10000,type='int',help='integer number of mcmc samples per walker')

        parser.add_option('--simname',type='string',help='DM simulation')

        parser.add_option('--halo_finder',default='rockstar',type='string',help='halo finder')

        parser.add_option('--redshift',default=0,type='float',help='redshift')

        parser.add_option('--version_name',type='string',help='version name')
	...
	hmfit=abhodfits.ABHodFitModel(datafile=input_data_file,covarfile=covar_file,simname=options.simname,version_name=options.version_name)
